...
Successful execution certifies the selected coordinate certificates and actual
physical input/upper-bound/chart helpers. It never certifies rank four.
"""
...
AGGREGATE = 'FermionicRank'
BLOCKS = ['RankCertificateSelfBlock' + str(i) for i in range(8)]
MODULES = ['Fock', 'Spinor', 'FockVacuumKernel', 'SpinorTransport', 'PureDecomposition',
           'PhysicalGaussian', 'PhysicalOccupancy', 'PhysicalGaussianRank',
           'GaussianScaling', 'FockParity', 'PureVacuumChart', 'PolynomialBoundary',
           'RankCertificate', 'RankCertificateTrace', 'RankCertificateNormal',
           'RankCertificateNormalCoordinates', 'RankCertificateCompression',
           'RankCertificateNormalSource', 'RankBitDomain', 'RankCertificateNormalTrace',
           'RankCertificateNormalAssembly', 'RankCertificateSelfPrelude']
MODULES += BLOCKS + ['RankCertificateNormalSelf', 'RankCertificateFullNormal',
                    'RankGramTransport']
CORE_ROOTS = [
    'Fermionic.RankCertificate.full_coordinate_target_certificate_nonzero',
    'Fermionic.RankCertificate.full_coordinate_normal_certificate',
    'Fermionic.PhysicalGaussian.gaussian_isPure',
    'Fermionic.PhysicalGaussian.decomposition_implies_pure',
    'Fermionic.PhysicalOccupancy.target_has_gaussian_decomposition',
    'Fermionic.PhysicalGaussianRank.targetGaussianRank_bounds',
    'Fermionic.PhysicalGaussianRank.targetPureRank_le_targetGaussianRank',
    'Fermionic.GaussianScaling.normalizedTarget_minimum',
    'Fermionic.FockParity.gaussian_even',
    'Fermionic.FockVacuumKernel.joint_annihilation_iff_vacuum',
    'Fermionic.RankCertificate.normal_source_partner_byte_even',
    'Fermionic.RankCertificate.target_source_partner_byte_even',
    'Fermionic.PureVacuumChart.graphDirection_two_contractions',
    'Fermionic.PolynomialBoundary.eq_zero_of_vanish_off_zero',
    'Fermionic.RankGramTransport.complete_gram_certificate_zero_iff']
STAMP = time.strftime('%Y%m%dT%H%M%SZ', time.gmtime())
RUN = ROOT / 'rank_verification_runs' / STAMP
CLEAN = RUN / 'clean'
LOGS = RUN / 'logs'
LOGS.mkdir(parents=True, exist_ok=False)
(RUN / 'verify_rank_remote_executed.py').write_bytes(VERIFIER_BYTES)
...
files = ['lean-toolchain', 'lakefile.toml', 'lake-manifest.json', AGGREGATE + '.lean']
...
# The eight independent row blocks use four workers; all dependencies are built first.
def compile_module(module):
    stem = 'Fermionic/' + module if module else AGGREGATE
    run(['lake', 'env', 'lean', '-DwarningAsError=true', '-o',
         '.lake/build/lib/lean/' + stem + '.olean', stem + '.lean'],
        CLEAN, 'compile-' + (module or 'aggregate') + '.log')
    print('Compiled ' + (module or AGGREGATE), flush=True)

for module in MODULES + ['']:
    if module == BLOCKS[0]:
        with ThreadPoolExecutor(max_workers=4) as pool:
            # Materialize every result, propagating every failed compilation.
            list(pool.map(compile_module, BLOCKS))
    elif module not in BLOCKS:
        compile_module(module)

audit = ['import ' + AGGREGATE, 'set_option pp.universes true',
...
run(['lake', 'env', 'leanchecker', '--fresh', '--verbose', AGGREGATE],
...
result = {
    ...
    'physical_gaussian_rank_four_verified': False,
    'all_three_pure_vanishing_verified': False,
    'actual_fock_coordinate_gram_identification_verified': False,
    'actual_normalization_covariance_verified': False,
    'full_coordinate_normal_family_verified': True,
    'physical_gaussian_input_and_upper_bound_verified': True,
    'actual_annihilator_vacuum_chart_verified': True,
    'actual_joint_annihilation_kernel_verified': True,
    'nonzero_scalar_decomposition_equivalence_verified': True,
    'coordinate_eight_bit_even_domain_verified': True,
    'core_roots': CORE_ROOTS,
    'scope': ('Original full coordinate normal-family polynomial vanishing for arbitrary '
              'complex a,b,c and target certificate nonvanishing; actual positive Euclidean '
              'Spin-vacuum Gaussian input is pure/even, and the unchanged target has a '
              'four-term Gaussian decomposition. Actual annihilator graph on the vacuum chart '
              'and general matrix covariance/polynomial-boundary helpers are included. '
              'No all-pure-triple normal form, actual Gram coordinate bridge, or rank-four '
              'lower bound is certified. This is an explicitly partial rank proof package.')
}
(RUN / 'result.json').write_text(json.dumps(result, indent=2, ensure_ascii=False) + '\n')
(ROOT / 'LATEST_RANK_RUN.txt').write_text(str(RUN) + '\n')
