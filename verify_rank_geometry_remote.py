#!/usr/bin/env python3
"""CAB-only clean geometry compilation, declaration audit and full kernel replay.

Successful execution certifies the actual physical input, scalar transport,
vacuum chart/kernel rigidity and original target rank at least two. It does
not certify coordinate polynomials, all three pure summands, or rank four.
"""
...
AGGREGATE = 'FermionicRankGeometry'
MODULES = ['Fock', 'Spinor', 'FockVacuumKernel', 'SpinorTransport', 'PureDecomposition',
           ...
           'GaussianScaling', 'FockParity', 'PureVacuumChart', 'PureVacuumRigidity']
CORE_ROOTS = [
    ...
    'Fermionic.PureVacuumRigidity.pure_zero_two_contractions_is_vacuum',
    'Fermionic.PureVacuumRigidity.target_not_pure',
    'Fermionic.PureVacuumRigidity.two_le_targetGaussianRank']
STAMP = time.strftime('%Y%m%dT%H%M%SZ', time.gmtime())
RUN = ROOT / 'geometry_verification_runs' / STAMP
CLEAN = RUN / 'clean'
LOGS = RUN / 'logs'
LOGS.mkdir(parents=True, exist_ok=False)
(RUN / 'verify_rank_geometry_remote_executed.py').write_bytes(VERIFIER_BYTES)
...
# The actual twelve-module geometric dependency closure is compiled sequentially.
...
result = {
    ...
    'full_coordinate_normal_family_verified': False,
    ...
    'coordinate_eight_bit_even_domain_verified': False,
    'original_vacuum_chart_zero_degree_two_rigidity_verified': True,
    'original_target_not_pure_verified': True,
    'original_target_rank_at_least_two_verified': True,
    'core_roots': CORE_ROOTS,
    'scope': ('Actual full exterior-algebra geometry only: positive Euclidean Spin-vacuum '
              'Gaussian cone is pure and even; unchanged original target admits four physical '
              'Gaussian summands; nonzero scalar transport preserves every decomposition length. '
              'The whole joint annihilation kernel is the vacuum line. The original nonzero '
              'vacuum pure-spinor chart is reconstructed as its full annihilator graph; zero '
              'degree-two contraction data force the vacuum. The unchanged target is not pure, '
              'so its physical Gaussian rank is at least two and at most four. '
              'No coordinate certificate, general exponential reconstruction, all-pure-triple '
              'normal form, actual Gram coordinate bridge, or rank-four lower bound is certified.')
}
(RUN / 'result.json').write_text(json.dumps(result, indent=2, ensure_ascii=False) + '\n')
(ROOT / 'LATEST_RANK_GEOMETRY_RUN.txt').write_text(str(RUN) + '\n')
